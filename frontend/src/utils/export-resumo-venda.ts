...
// Caixa). Faturamento/custo/margem por nível de produto (breadcrumb
// completo, [GLOBAL] "Nível — Sempre Exibir o Caminho Completo").
import { Platform } from "react-native";
import * as Print from "expo-print";
import * as Sharing from "expo-sharing";
import { buildReportHeaderHtml, EmpresaHeader, REPORT_HEADER_CSS } from "./print-report-header";

export type ResumoVendaNivel = {
  codigo: string;
  label: string;
  venda: number;
  custo: number;
...
export type ResumoVendaPayload = {
  titulo: string;
  periodo: string;
  niveis: ResumoVendaNivel[];
  totais: { venda: number; custo: number; margem: number; margem_pct: number };
...
function buildHtml(p: ResumoVendaPayload): string {
  const geradoEm = new Date().toLocaleString("pt-BR");
  const linhas = p.niveis
    .map(
      (n) => `
        <tr>
          <td>${esc(n.label || "Sem Classificação")}</td>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    <table>
      ...
      <tfoot><tr><td>FATURAMENTO TOTAL</td><td class="num">${brl(p.totais.venda)}</td><td class="num">${brl(p.totais.custo)}</td><td class="num">${brl(p.totais.margem)}</td><td class="num">${pct(p.totais.margem_pct)}</td></tr></tfoot>
    </table>
  </body></html>`;
}

export async function exportResumoVendaPdf(payload: ResumoVendaPayload): Promise<void> {
