...
type Situacao = { codigo: string; descricao: string };

// Cadastro/Tabelas Auxiliares > Situação (tabela `situacao`) — situações
// genéricas (ex.: A=Ativo, I=Inativo) referenciadas como texto livre por
// vários campos do sistema (não é a mesma tabela de STATUS_CLIENTE). Código
// é um mnemônico curto digitado pelo usuário — upsert-by-codigo, travado
// depois de criado.
export default function SituacaoScreen() {
  ...
  if (!isWeb) {
    return (
      <LockedView
        ...
        message="Situação está disponível apenas no web."
        testID="situacao-web-only"
      />
    );
  }
  ...
  const load = useCallback(async (c: Conn, q: string) => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/tabelas/situacao?${qs}`);
  ...
  const openEdit = (s: Situacao) => { setEditCod(s.codigo); setCodigo(s.codigo); setDescricao(s.descricao); setFormOpen(true); };
  ...
  const save = async () => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/tabelas/situacao`, {
      ...
      const j = await r.json();
      if (j?.success) { showToast(j.message || "Situação gravada."); setFormOpen(false); load(conn, search); }
  ...
  const remove = async (s: Situacao) => {
    if (!conn) return;
    try {
      const base = conn.api.replace(/\/+$/, "");
      const r = await fetch(`${base}/api/tabelas/situacao/${encodeURIComponent(s.codigo)}/excluir`, {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const canSave = can("SITUACAO.GRAVAR") || isMaster;
  const canDel = can("SITUACAO.EXCLUIR") || isMaster;

  return (
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]} testID="situacao-screen">
      ...
      <View style={styles.webShell}>
        <View style={styles.filterBox}>
          <TextInput
            ...
            style={styles.input}
            testID="situacao-search"
          />
        </View>

        <ScrollView contentContainerStyle={[styles.scroll, styles.scrollWeb]}>
          {loading ? <ActivityIndicator color={colors.brandPrimary} style={{ marginTop: 24 }} /> : null}
          {!loading && items.length === 0 ? <Text style={styles.empty}>Nenhuma situação cadastrada.</Text> : null}
          {items.map((s) => (
            <View key={s.codigo} style={styles.row} testID={`situacao-${s.codigo}`}>
              <Pressable style={{ flex: 1 }} onPress={() => canSave && openEdit(s)}>
                <Text style={styles.rowTitle}>{s.codigo} · {s.descricao}</Text>
              </Pressable>
              {canDel ? (
                <Pressable onPress={() => remove(s)} hitSlop={8} testID={`situacao-del-${s.codigo}`}>
      ...
      {canSave ? (
        <Pressable onPress={openNew} style={styles.fab} testID="situacao-novo">
      ...
      <Modal visible={formOpen} transparent animationType="slide" onRequestClose={() => setFormOpen(false)}>
        <Pressable style={styles.modalBg} onPress={() => setFormOpen(false)}>
          <Pressable style={styles.modalCard} onPress={(e) => e.stopPropagation()}>
            <Text style={styles.modalTitle}>{editCod ? `Situação ${editCod}` : "Nova situação"}</Text>
            ...
            <TextInput
              ...
              style={styles.input}
              maxLength={30}
              testID="situacao-descricao"
            />

            <Pressable onPress={save} disabled={saving} style={[styles.primaryBtn, saving && { opacity: 0.6 }]} testID="situacao-salvar">
