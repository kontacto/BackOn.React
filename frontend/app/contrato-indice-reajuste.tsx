...
// Transações > Contratos > Índices de Reajuste (tabela `indice_reajuste`,
// migração de FrmManInd.frm) — mesmo padrão de tipo-cliente.tsx.
export default function ContratoIndiceReajusteScreen() {
  ...
  if (!isWeb) {
    return (
      <LockedView
        ...
        message="Índices de Reajuste está disponível apenas no web."
        testID="contrato-indice-reajuste-web-only"
      />
    );
  }
  ...
  if (!moduleOn("contratos")) {
    return (
      <LockedView
        ...
        testID="contrato-indice-reajuste-module-off"
      />
    );
  }
  ...
  const load = useCallback(async (c: Conn, q: string) => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/contratos/indice-reajuste?${qs}`);
  ...
  const save = async () => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/contratos/indice-reajuste`, {
  ...
  const remove = async (t: Tipo) => {
    ...
    try {
      const base = conn.api.replace(/\/+$/, "");
      const r = await fetch(`${base}/api/contratos/indice-reajuste/${t.codigo}/excluir`, {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const canSave = can("INDICE_REAJUSTE.GRAVAR") || isMaster;
  const canDel = can("INDICE_REAJUSTE.EXCLUIR") || isMaster;

  return (
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]} testID="contrato-indice-reajuste-screen">
      <View style={styles.header}>
        <Pressable onPress={() => router.back()} hitSlop={12} style={styles.back}>
          <Ionicons name="chevron-back" size={24} color={colors.onBrandPrimary} />
        </Pressable>
        <Text style={styles.headerTitle}>Índices de Reajuste</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={styles.webShell}>
        <View style={styles.filterBox}>
          <TextInput
            ...
            style={styles.input}
            testID="contrato-indice-reajuste-search"
          />
        </View>
        ...
        <ScrollView contentContainerStyle={[styles.scroll, styles.scrollWeb]}>
          ...
          {items.map((t) => (
            <View key={t.codigo} style={styles.row} testID={`contrato-indice-reajuste-${t.codigo}`}>
              <Pressable style={{ flex: 1 }} onPress={() => canSave && openEdit(t)}>
                <Text style={styles.rowTitle}>{t.codigo} · {t.descricao}</Text>
              </Pressable>
              {canDel ? (
                <Pressable onPress={() => remove(t)} hitSlop={8} testID={`contrato-indice-reajuste-del-${t.codigo}`}>
      ...
      {canSave ? (
        <Pressable onPress={openNew} style={styles.fab} testID="contrato-indice-reajuste-novo">
        ...
        <Pressable style={styles.modalBg} onPress={() => setFormOpen(false)}>
          <Pressable style={styles.modalCard} onPress={(e) => e.stopPropagation()}>
            ...
            <TextInput
              ...
              testID="contrato-indice-reajuste-descricao"
            />
            <Pressable onPress={save} disabled={saving} style={[styles.primaryBtn, saving && { opacity: 0.6 }]} testID="contrato-indice-reajuste-salvar">
