...
type Grupo = { codigo: string; descricao: string };

// Cadastro/Tabelas Auxiliares > Grupo PIS/COFINS (tabela `grupo_pis_cofins`,
// coluna real `cod_grupo`). Agrupamento definido pela própria empresa (não é
// vocabulário fixo da legislação) — código auto-gerado (3 dígitos
// sequenciais), mesmo padrão de Marcas/Segmentos.
export default function GrupoPisCofinsScreen() {
  ...
  if (!isWeb) {
    return (
      <LockedView
        ...
        message="Grupo PIS/COFINS está disponível apenas no web."
        testID="grupo-pis-cofins-web-only"
      />
    );
  }
  ...
  const load = useCallback(async (c: Conn, q: string) => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/tabelas/grupo-pis-cofins?${qs}`);
  ...
  const openEdit = (g: Grupo) => { setEditCod(g.codigo); setDescricao(g.descricao); setFormOpen(true); };
  ...
  const save = async () => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/tabelas/grupo-pis-cofins`, {
  ...
  const remove = async (g: Grupo) => {
    ...
    try {
      const base = conn.api.replace(/\/+$/, "");
      const r = await fetch(`${base}/api/tabelas/grupo-pis-cofins/${encodeURIComponent(g.codigo)}/excluir`, {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const canSave = can("GRUPO_PISCOF.GRAVAR") || isMaster;
  const canDel = can("GRUPO_PISCOF.EXCLUIR") || isMaster;

  return (
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]} testID="grupo-pis-cofins-screen">
      <View style={styles.header}>
        <Pressable onPress={() => router.back()} hitSlop={12} style={styles.back}>
          <Ionicons name="chevron-back" size={24} color={colors.onBrandPrimary} />
        </Pressable>
        <Image source={require("../assets/images/kontacto-logo.png")} style={{ width: 56, height: 16, marginRight: 8 }} resizeMode="contain" />
        <Text style={styles.headerTitle}>Grupo PIS/COFINS</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={styles.webShell}>
        <View style={styles.filterBox}>
          <TextInput
            ...
            style={styles.input}
            testID="grupo-pis-cofins-search"
          />
        </View>
        ...
        <ScrollView contentContainerStyle={[styles.scroll, styles.scrollWeb]}>
          ...
          {items.map((g) => (
            <View key={g.codigo} style={styles.row} testID={`grupo-pis-cofins-${g.codigo}`}>
              <Pressable style={{ flex: 1 }} onPress={() => canSave && openEdit(g)}>
                <Text style={styles.rowTitle}>{g.codigo} · {g.descricao}</Text>
              </Pressable>
              {canDel ? (
                <Pressable onPress={() => remove(g)} hitSlop={8} testID={`grupo-pis-cofins-del-${g.codigo}`}>
      ...
      {canSave ? (
        <Pressable onPress={openNew} style={styles.fab} testID="grupo-pis-cofins-novo">
        ...
        <Pressable style={styles.modalBg} onPress={() => setFormOpen(false)}>
          <Pressable style={styles.modalCard} onPress={(e) => e.stopPropagation()}>
            ...
            <TextInput
              ...
              placeholder="Ex.: Tributado Normal"
              placeholderTextColor={colors.muted}
              style={styles.input}
              maxLength={35}
              autoCapitalize="characters"
              testID="grupo-pis-cofins-descricao"
            />
            <Pressable onPress={save} disabled={saving} style={[styles.primaryBtn, saving && { opacity: 0.6 }]} testID="grupo-pis-cofins-salvar">
