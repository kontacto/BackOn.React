...
type Regiao = { codigo: number; descricao: string };

// Cadastro/Tabelas Auxiliares > Regiões (tabela `regioes`). Código não é
// digitado — gerado automaticamente (MAX+1), mesmo padrão de Marcas/Área.
// Delete guard real: cliente.regiao é gravado por este app (cliente-completo).
export default function RegioesScreen() {
  ...
  if (!isWeb) {
    return (
      <LockedView
        ...
        message="Regiões está disponível apenas no web."
        testID="regioes-web-only"
      />
    );
  }
  ...
  const load = useCallback(async (c: Conn, q: string) => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/tabelas/regioes?${qs}`);
  ...
  const openEdit = (r: Regiao) => { setEditCod(r.codigo); setDescricao(r.descricao); setFormOpen(true); };
  ...
  const save = async () => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/tabelas/regioes`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ servidor: conn.servidor, banco: conn.banco, ...auditCtx, codigo: editCod, descricao: descricao.trim() }),
      });
      const j = await r.json();
      if (j?.success) { showToast(j.message || "Região gravada."); setFormOpen(false); load(conn, search); }
  ...
  const remove = async (r: Regiao) => {
    if (!conn) return;
    try {
      const base = conn.api.replace(/\/+$/, "");
      const resp = await fetch(`${base}/api/tabelas/regioes/${r.codigo}/excluir`, {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const canSave = can("REGIOES.GRAVAR") || isMaster;
  const canDel = can("REGIOES.EXCLUIR") || isMaster;

  return (
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]} testID="regioes-screen">
      ...
      <View style={styles.webShell}>
        <View style={styles.filterBox}>
          <TextInput
            ...
            style={styles.input}
            testID="regioes-search"
          />
        </View>

        <ScrollView contentContainerStyle={[styles.scroll, styles.scrollWeb]}>
          {loading ? <ActivityIndicator color={colors.brandPrimary} style={{ marginTop: 24 }} /> : null}
          {!loading && items.length === 0 ? <Text style={styles.empty}>Nenhuma região cadastrada.</Text> : null}
          {items.map((r) => (
            <View key={r.codigo} style={styles.row} testID={`regiao-${r.codigo}`}>
              <Pressable style={{ flex: 1 }} onPress={() => canSave && openEdit(r)}>
                <Text style={styles.rowTitle}>{r.codigo} · {r.descricao}</Text>
              </Pressable>
              {canDel ? (
                <Pressable onPress={() => remove(r)} hitSlop={8} testID={`regiao-del-${r.codigo}`}>
      ...
      {canSave ? (
        <Pressable onPress={openNew} style={styles.fab} testID="regioes-novo">
      ...
      <Modal visible={formOpen} transparent animationType="slide" onRequestClose={() => setFormOpen(false)}>
        <Pressable style={styles.modalBg} onPress={() => setFormOpen(false)}>
          <Pressable style={styles.modalCard} onPress={(e) => e.stopPropagation()}>
            <Text style={styles.modalTitle}>{editCod != null ? `Região ${editCod}` : "Nova região"}</Text>
            ...
            <TextInput
              ...
              testID="regiao-descricao"
            />
            <Pressable onPress={save} disabled={saving} style={[styles.primaryBtn, saving && { opacity: 0.6 }]} testID="regiao-salvar">
