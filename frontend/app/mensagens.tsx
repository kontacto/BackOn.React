...
// Cadastro/Tabelas Auxiliares > Mensagens (tabela `Mensagens`). Legado:
// FrmManMsg ("Manutenção de Mensagens") — textos padronizados usados em
// observação de nota fiscal/orçamento. Código não é digitado — gerado
// automaticamente (IDENTITY), mesmo padrão de Regiões/Tipo Cliente.
export default function MensagensScreen() {
  ...
  if (!isWeb) {
    return (
      <LockedView
        ...
        message="Mensagens está disponível apenas no web."
        testID="mensagens-web-only"
      />
    );
  }
  ...
  const load = useCallback(async (c: Conn, q: string) => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/tabelas/mensagens?${qs}`);
  ...
  const openEdit = (m: Mensagem) => { setEditCod(m.codigo); setDescricao(m.descricao); setFormOpen(true); };
  ...
  const save = async () => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/tabelas/mensagens`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ servidor: conn.servidor, banco: conn.banco, ...auditCtx, codigo: editCod, descricao: descricao.trim() }),
      });
      const j = await r.json();
      if (j?.success) { showToast(j.message || "Mensagem gravada."); setFormOpen(false); load(conn, search); }
  ...
  const remove = async (m: Mensagem) => {
    if (!conn) return;
    try {
      const base = conn.api.replace(/\/+$/, "");
      const r = await fetch(`${base}/api/tabelas/mensagens/${m.codigo}/excluir`, {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const canSave = can("MENSAGENS.GRAVAR") || isMaster;
  const canDel = can("MENSAGENS.EXCLUIR") || isMaster;

  return (
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]} testID="mensagens-screen">
      ...
      <View style={styles.webShell}>
        <View style={styles.filterBox}>
          <TextInput
            ...
            style={styles.input}
            testID="mensagens-search"
          />
        </View>

        <ScrollView contentContainerStyle={[styles.scroll, styles.scrollWeb]}>
          {loading ? <ActivityIndicator color={colors.brandPrimary} style={{ marginTop: 24 }} /> : null}
          {!loading && items.length === 0 ? <Text style={styles.empty}>Nenhuma mensagem cadastrada.</Text> : null}
          {items.map((m) => (
            <View key={m.codigo} style={styles.row} testID={`mensagem-${m.codigo}`}>
              <Pressable style={{ flex: 1 }} onPress={() => canSave && openEdit(m)}>
                <Text style={styles.rowTitle} numberOfLines={2}>{m.codigo} · {m.descricao}</Text>
              </Pressable>
              {canDel ? (
                <Pressable onPress={() => remove(m)} hitSlop={8} testID={`mensagem-del-${m.codigo}`}>
      ...
      {canSave ? (
        <Pressable onPress={openNew} style={styles.fab} testID="mensagens-novo">
      ...
      <Modal visible={formOpen} transparent animationType="slide" onRequestClose={() => setFormOpen(false)}>
        <Pressable style={styles.modalBg} onPress={() => setFormOpen(false)}>
          <Pressable style={styles.modalCard} onPress={(e) => e.stopPropagation()}>
            <Text style={styles.modalTitle}>{editCod != null ? `Mensagem ${editCod}` : "Nova mensagem"}</Text>
            <Text style={styles.label}>Descrição</Text>
            <TextInput
              value={descricao}
              onChangeText={setDescricao}
              placeholder="Ex.: EPP enquadrada no Simples - Lei nº 9.317/96."
              placeholderTextColor={colors.muted}
              style={[styles.input, styles.textArea]}
              multiline
              numberOfLines={6}
              testID="mensagem-descricao"
            />
            <Pressable onPress={save} disabled={saving} style={[styles.primaryBtn, saving && { opacity: 0.6 }]} testID="mensagem-salvar">
