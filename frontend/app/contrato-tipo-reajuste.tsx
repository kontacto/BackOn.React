...
// Transações > Contratos > Tipo de Reajuste (tabela `prazo_reajuste`,
// migração de FrmManRea.frm) — mesmo padrão de tipo-cliente.tsx.
export default function ContratoTipoReajusteScreen() {
  ...
  if (!isWeb) {
    return (
      <LockedView
        ...
        message="Tipo de Reajuste está disponível apenas no web."
        testID="contrato-tipo-reajuste-web-only"
      />
    );
  }
  ...
  if (!moduleOn("contratos")) {
    return (
      <LockedView
        ...
        testID="contrato-tipo-reajuste-module-off"
      />
    );
  }
  ...
  const load = useCallback(async (c: Conn, q: string) => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/contratos/tipo-reajuste?${qs}`);
  ...
  const save = async () => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/contratos/tipo-reajuste`, {
  ...
  const remove = async (t: Tipo) => {
    ...
    try {
      const base = conn.api.replace(/\/+$/, "");
      const r = await fetch(`${base}/api/contratos/tipo-reajuste/${t.codigo}/excluir`, {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const canSave = can("TIPO_REAJUSTE.GRAVAR") || isMaster;
  const canDel = can("TIPO_REAJUSTE.EXCLUIR") || isMaster;

  return (
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]} testID="contrato-tipo-reajuste-screen">
      <View style={styles.header}>
        <Pressable onPress={() => router.back()} hitSlop={12} style={styles.back}>
          <Ionicons name="chevron-back" size={24} color={colors.onBrandPrimary} />
        </Pressable>
        <Text style={styles.headerTitle}>Tipo de Reajuste</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={styles.webShell}>
        <View style={styles.filterBox}>
          <TextInput
            ...
            style={styles.input}
            testID="contrato-tipo-reajuste-search"
          />
        </View>
        ...
        <ScrollView contentContainerStyle={[styles.scroll, styles.scrollWeb]}>
          ...
          {items.map((t) => (
            <View key={t.codigo} style={styles.row} testID={`contrato-tipo-reajuste-${t.codigo}`}>
              <Pressable style={{ flex: 1 }} onPress={() => canSave && openEdit(t)}>
                <Text style={styles.rowTitle}>{t.codigo} · {t.descricao}</Text>
              </Pressable>
              {canDel ? (
                <Pressable onPress={() => remove(t)} hitSlop={8} testID={`contrato-tipo-reajuste-del-${t.codigo}`}>
      ...
      {canSave ? (
        <Pressable onPress={openNew} style={styles.fab} testID="contrato-tipo-reajuste-novo">
        ...
        <Pressable style={styles.modalBg} onPress={() => setFormOpen(false)}>
          <Pressable style={styles.modalCard} onPress={(e) => e.stopPropagation()}>
            ...
            <TextInput
              ...
              testID="contrato-tipo-reajuste-descricao"
            />
            <Pressable onPress={save} disabled={saving} style={[styles.primaryBtn, saving && { opacity: 0.6 }]} testID="contrato-tipo-reajuste-salvar">
