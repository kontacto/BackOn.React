...
type Unid = { codigo: string; descricao: string; permite_decimais: boolean };

// Cadastro/Tabelas Auxiliares > Unidade de Medida (tabela `unid`). Legado:
// FrmManUni ("Manutenção de Unidades de Medidas"), campo "Código" — código É
// digitado pelo usuário (upsert-by-codigo, mesmo padrão de Situação/Icms/
// Origem/Status de O.S./Tipo de Pré-Venda/Tipo de Produto/Tipo de Serviço/
// Tributação), travado depois de criado. Diferente de Tributação, o Gravar
// original aqui salva os dois campos (Descrição e Permite Decimais) direito
// — sem bug de campo esquecido.
export default function UnidadeMedidaScreen() {
  ...
  if (!isWeb) {
    return (
      <LockedView
        ...
        message="Unidade de Medida está disponível apenas no web."
        testID="unidade-medida-web-only"
      />
    );
  }

  const [conn, setConn] = useState<Conn | null>(null);
  const [items, setItems] = useState<Unid[]>([]);
  ...
  const [permiteDecimais, setPermiteDecimais] = useState(false);
  ...
  const openNew = () => { setEditCod(null); setCodigo(""); setDescricao(""); setPermiteDecimais(false); setFormOpen(true); };
  const openEdit = (u: Unid) => {
    setEditCod(u.codigo); setCodigo(u.codigo); setDescricao(u.descricao); setPermiteDecimais(u.permite_decimais); setFormOpen(true);
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/tabelas/unid`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          servidor: conn.servidor, banco: conn.banco, ...auditCtx,
          codigo: codigo.trim(), descricao: descricao.trim(), permite_decimais: permiteDecimais,
        }),
      });
      const j = await r.json();
      if (j?.success) { showToast(j.message || "Unidade gravada."); setFormOpen(false); load(conn, search); }
  ...
  const remove = async (u: Unid) => {
    if (!conn) return;
    try {
      const base = conn.api.replace(/\/+$/, "");
      const r = await fetch(`${base}/api/tabelas/unid/${encodeURIComponent(u.codigo)}/excluir`, {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const canSave = can("UNID.GRAVAR") || isMaster;
  const canDel = can("UNID.EXCLUIR") || isMaster;

  return (
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]} testID="unidade-medida-screen">
      <View style={styles.header}>
        <Pressable onPress={() => router.back()} hitSlop={12} style={styles.back}>
          <Ionicons name="chevron-back" size={24} color={colors.onBrandPrimary} />
        </Pressable>
        <Image source={require("../assets/images/kontacto-logo.png")} style={{ width: 56, height: 16, marginRight: 8 }} resizeMode="contain" />
        <Text style={styles.headerTitle}>Unidade de Medida</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={styles.webShell}>
        <View style={styles.filterBox}>
          <TextInput
            ...
            style={styles.input}
            testID="unidade-medida-search"
          />
        </View>

        <ScrollView contentContainerStyle={[styles.scroll, styles.scrollWeb]}>
          {loading ? <ActivityIndicator color={colors.brandPrimary} style={{ marginTop: 24 }} /> : null}
          {!loading && items.length === 0 ? <Text style={styles.empty}>Nenhuma unidade cadastrada.</Text> : null}
          {items.map((u) => (
            <View key={u.codigo} style={styles.row} testID={`unidade-medida-${u.codigo}`}>
              <Pressable style={{ flex: 1 }} onPress={() => canSave && openEdit(u)}>
                <Text style={styles.rowTitle}>{u.codigo} · {u.descricao}</Text>
                <Text style={styles.rowSub}>{u.permite_decimais ? "Permite decimais" : "Não permite decimais"}</Text>
              </Pressable>
              {canDel ? (
                <Pressable onPress={() => remove(u)} hitSlop={8} testID={`unidade-medida-del-${u.codigo}`}>
      ...
      {canSave ? (
        <Pressable onPress={openNew} style={styles.fab} testID="unidade-medida-novo">
      ...
      <Modal visible={formOpen} transparent animationType="slide" onRequestClose={() => setFormOpen(false)}>
        <Pressable style={styles.modalBg} onPress={() => setFormOpen(false)}>
          <Pressable style={styles.modalCard} onPress={(e) => e.stopPropagation()}>
            <Text style={styles.modalTitle}>{editCod ? `Unidade ${editCod}` : "Nova unidade"}</Text>
            <ScrollView showsVerticalScrollIndicator={false}>
              <Text style={styles.label}>Código *</Text>
              <TextInput
                value={codigo}
                onChangeText={(v) => setCodigo(v.toUpperCase())}
                placeholder="Ex.: CX, DZ, MT"
                placeholderTextColor={colors.muted}
                style={[styles.input, editCod != null && styles.inputDisabled]}
                editable={editCod == null}
                autoCapitalize="characters"
                maxLength={3}
                testID="unidade-medida-codigo"
              />
              <Text style={styles.label}>Descrição</Text>
              <TextInput
                value={descricao}
                onChangeText={setDescricao}
                placeholder="Ex.: CAIXA, DUZIA, METRO"
                placeholderTextColor={colors.muted}
                style={styles.input}
                maxLength={12}
                testID="unidade-medida-descricao"
              />
              <View style={styles.switchRow}>
                <Text style={styles.switchLabel}>Permite Decimais</Text>
                <Switch value={permiteDecimais} onValueChange={setPermiteDecimais} testID="unidade-medida-permite-decimais" />
              </View>
              <Pressable onPress={save} disabled={saving} style={[styles.primaryBtn, saving && { opacity: 0.6 }]} testID="unidade-medida-salvar">
...
const styles = StyleSheet.create({
  ...
  switchRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingVertical: 4, marginTop: spacing.sm },
