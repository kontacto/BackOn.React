...
type TipoDoc = { codigo: number; descricao: string };

// Cadastro/Tabelas Auxiliares > Tipo de Documento (tabela `tipo_doc`). Código
// não é digitado — gerado automaticamente (MAX+1), mesmo padrão de
// Regiões/Tipo Cliente. Nenhuma tabela deste app usa isso ainda (emissão de
// NF não está implementada), guard de exclusão é só existência.
export default function TipoDocScreen() {
  ...
  if (!isWeb) {
    return (
      <LockedView
        ...
        message="Tipo de Documento está disponível apenas no web."
        testID="tipo-doc-web-only"
      />
    );
  }

  const [conn, setConn] = useState<Conn | null>(null);
  const [items, setItems] = useState<TipoDoc[]>([]);
  ...
  const load = useCallback(async (c: Conn, q: string) => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/tabelas/tipo-doc?${qs}`);
  ...
  const openEdit = (t: TipoDoc) => { setEditCod(t.codigo); setDescricao(t.descricao); setFormOpen(true); };
  ...
  const save = async () => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/tabelas/tipo-doc`, {
  ...
  const remove = async (t: TipoDoc) => {
    if (!conn) return;
    try {
      const base = conn.api.replace(/\/+$/, "");
      const r = await fetch(`${base}/api/tabelas/tipo-doc/${t.codigo}/excluir`, {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const canSave = can("TIPO_DOC.GRAVAR") || isMaster;
  const canDel = can("TIPO_DOC.EXCLUIR") || isMaster;

  return (
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]} testID="tipo-doc-screen">
      <View style={styles.header}>
        <Pressable onPress={() => router.back()} hitSlop={12} style={styles.back}>
          <Ionicons name="chevron-back" size={24} color={colors.onBrandPrimary} />
        </Pressable>
        <Image source={require("../assets/images/kontacto-logo.png")} style={{ width: 56, height: 16, marginRight: 8 }} resizeMode="contain" />
        <Text style={styles.headerTitle}>Tipo de Documento</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={styles.webShell}>
        ...
        <ScrollView contentContainerStyle={[styles.scroll, styles.scrollWeb]}>
          ...
          {items.map((t) => (
            <View key={t.codigo} style={styles.row} testID={`tipo-doc-${t.codigo}`}>
              <Pressable style={{ flex: 1 }} onPress={() => canSave && openEdit(t)}>
                <Text style={styles.rowTitle}>{t.codigo} · {t.descricao}</Text>
              </Pressable>
              {canDel ? (
                <Pressable onPress={() => remove(t)} hitSlop={8} testID={`tipo-doc-del-${t.codigo}`}>
      ...
      {canSave ? (
        <Pressable onPress={openNew} style={styles.fab} testID="tipo-doc-novo">
        ...
        <Pressable style={styles.modalBg} onPress={() => setFormOpen(false)}>
          <Pressable style={styles.modalCard} onPress={(e) => e.stopPropagation()}>
            ...
            <TextInput
              ...
              testID="tipo-doc-descricao"
            />
            <Pressable onPress={save} disabled={saving} style={[styles.primaryBtn, saving && { opacity: 0.6 }]} testID="tipo-doc-salvar">
