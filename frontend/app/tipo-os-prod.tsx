...
type TipoOsProd = { codigo: number; descricao: string };

// Cadastro/Tabelas Auxiliares > Tipo Destino Itens OS (tabela `tipo_os_prod`).
// Legado: FrmManTipos ("Manutenção Tipo de OS Produto"), campo "Tipo" —
// código É digitado pelo usuário (upsert-by-codigo, mesmo padrão de
// Situação/Icms/Origem/Tipo de Serviço/Tipo de Produto), travado depois de
// criado. Classifica o destino de cada item (produto) de uma O.S. —
// Cliente/Garantia/Interno/Revisão de Fábrica no banco de teste.
//
// Delete guard: FK real `FK_os_produto_tipo_os_prod` — a coluna
// `os_produto.situacao` (nome enganoso: não é a situação da O.S., é o
// código deste tipo — confirmado via sys.foreign_keys) referencia esta
// tabela.
export default function TipoOsProdScreen() {
  ...
  if (!isWeb) {
    return (
      <LockedView
        ...
        message="Tipo Destino Itens OS está disponível apenas no web."
        testID="tipo-os-prod-web-only"
      />
    );
  }

  const [conn, setConn] = useState<Conn | null>(null);
  const [items, setItems] = useState<TipoOsProd[]>([]);
  ...
  const openEdit = (t: TipoOsProd) => { setEditCod(t.codigo); setCodigo(String(t.codigo)); setDescricao(t.descricao); setFormOpen(true); };

  const save = async () => {
    if (!conn) return;
    if (!codigo.trim()) { showToast("O código não pode ficar em branco!"); return; }
    if (!descricao.trim()) { showToast("A descrição não pode ficar em branco!"); return; }
    setSaving(true);
    try {
      const base = conn.api.replace(/\/+$/, "");
      const r = await fetch(`${base}/api/tabelas/tipo-os-prod`, {
      ...
      const j = await r.json();
      if (j?.success) { showToast(j.message || "Registro gravado com sucesso."); setFormOpen(false); load(conn, search); }
  ...
  const remove = async (t: TipoOsProd) => {
    if (!conn) return;
    try {
      const base = conn.api.replace(/\/+$/, "");
      const r = await fetch(`${base}/api/tabelas/tipo-os-prod/${t.codigo}/excluir`, {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const canSave = can("TIPO_OS_PROD.GRAVAR") || isMaster;
  const canDel = can("TIPO_OS_PROD.EXCLUIR") || isMaster;

  return (
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]} testID="tipo-os-prod-screen">
      <View style={styles.header}>
        <Pressable onPress={() => router.back()} hitSlop={12} style={styles.back}>
          <Ionicons name="chevron-back" size={24} color={colors.onBrandPrimary} />
        </Pressable>
        <Image source={require("../assets/images/kontacto-logo.png")} style={{ width: 56, height: 16, marginRight: 8 }} resizeMode="contain" />
        <Text style={styles.headerTitle}>Tipo Destino Itens OS</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={styles.webShell}>
        ...
        <ScrollView contentContainerStyle={[styles.scroll, styles.scrollWeb]}>
          ...
          {items.map((t) => (
            <View key={t.codigo} style={styles.row} testID={`tipo-os-prod-${t.codigo}`}>
              <Pressable style={{ flex: 1 }} onPress={() => canSave && openEdit(t)}>
                <Text style={styles.rowTitle}>{t.codigo} · {t.descricao}</Text>
              </Pressable>
              {canDel ? (
                <Pressable onPress={() => remove(t)} hitSlop={8} testID={`tipo-os-prod-del-${t.codigo}`}>
      ...
      {canSave ? (
        <Pressable onPress={openNew} style={styles.fab} testID="tipo-os-prod-novo">
        ...
        <Pressable style={styles.modalBg} onPress={() => setFormOpen(false)}>
          <Pressable style={styles.modalCard} onPress={(e) => e.stopPropagation()}>
            ...
            <TextInput
              ...
              maxLength={4}
              testID="tipo-os-prod-codigo"
            />
            <Text style={styles.label}>Descrição</Text>
            <TextInput
              value={descricao}
              onChangeText={setDescricao}
              placeholder="Ex.: Cliente, Garantia"
              placeholderTextColor={colors.muted}
              style={styles.input}
              maxLength={30}
              testID="tipo-os-prod-descricao"
            />
            <Pressable onPress={save} disabled={saving} style={[styles.primaryBtn, saving && { opacity: 0.6 }]} testID="tipo-os-prod-salvar">
