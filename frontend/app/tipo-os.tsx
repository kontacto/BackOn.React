...
type TipoOs = { codigo: number; descricao: string };

// Cadastro/Tabelas Auxiliares > Tipo de Pré-Venda (tabela `tipo_os`). Legado:
// FrmManTipoOS ("Tipos de O.S."), campo "Tipo" — código É digitado pelo
// usuário (upsert-by-codigo, mesmo padrão de Situação/Icms/Origem/Status de
// O.S.), travado depois de criado. Renomeado pra "Tipo de Pré-Venda" porque
// `tipo_os.codigo` tem FK real tanto de `os.tipo` quanto de
// `pedido_venda.tipo` — classifica o tipo de AMBOS os fluxos de pré-venda
// deste app, não só O.S. Delete guard real: bloqueia se houver O.S. ou
// Pedido usando o tipo.
export default function TipoOsScreen() {
  ...
  if (!isWeb) {
    return (
      <LockedView
        ...
        message="Tipo de Pré-Venda está disponível apenas no web."
        testID="tipo-os-web-only"
      />
    );
  }

  const [conn, setConn] = useState<Conn | null>(null);
  const [items, setItems] = useState<TipoOs[]>([]);
  ...
  const openEdit = (t: TipoOs) => { setEditCod(t.codigo); setCodigo(String(t.codigo)); setDescricao(t.descricao); setFormOpen(true); };
  ...
  const save = async () => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/tabelas/tipo-os`, {
  ...
  const remove = async (t: TipoOs) => {
    if (!conn) return;
    try {
      const base = conn.api.replace(/\/+$/, "");
      const r = await fetch(`${base}/api/tabelas/tipo-os/${t.codigo}/excluir`, {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const canSave = can("TIPO_OS.GRAVAR") || isMaster;
  const canDel = can("TIPO_OS.EXCLUIR") || isMaster;

  return (
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]} testID="tipo-os-screen">
      <View style={styles.header}>
        <Pressable onPress={() => router.back()} hitSlop={12} style={styles.back}>
          <Ionicons name="chevron-back" size={24} color={colors.onBrandPrimary} />
        </Pressable>
        <Image source={require("../assets/images/kontacto-logo.png")} style={{ width: 56, height: 16, marginRight: 8 }} resizeMode="contain" />
        <Text style={styles.headerTitle}>Tipo de Pré-Venda</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={styles.webShell}>
        ...
        <ScrollView contentContainerStyle={[styles.scroll, styles.scrollWeb]}>
          ...
          {items.map((t) => (
            <View key={t.codigo} style={styles.row} testID={`tipo-os-${t.codigo}`}>
              <Pressable style={{ flex: 1 }} onPress={() => canSave && openEdit(t)}>
                <Text style={styles.rowTitle}>{t.codigo} · {t.descricao}</Text>
              </Pressable>
              {canDel ? (
                <Pressable onPress={() => remove(t)} hitSlop={8} testID={`tipo-os-del-${t.codigo}`}>
      ...
      {canSave ? (
        <Pressable onPress={openNew} style={styles.fab} testID="tipo-os-novo">
        ...
        <Pressable style={styles.modalBg} onPress={() => setFormOpen(false)}>
          <Pressable style={styles.modalCard} onPress={(e) => e.stopPropagation()}>
            ...
            <TextInput
              ...
              testID="tipo-os-descricao"
            />
            <Pressable onPress={save} disabled={saving} style={[styles.primaryBtn, saving && { opacity: 0.6 }]} testID="tipo-os-salvar">
