...
type Icms = { codigo: string; descricao: string };

// Cadastro/Tabelas Auxiliares > Icms (tabela `dscr_icms`) — situações
// tributárias de ICMS (CST/CSOSN). Código é o próprio código fiscal
// padronizado (ex.: "00", "20", "101"), digitado pelo usuário — não é
// sequencial arbitrário, então fica travado depois de criado (upsert-by-codigo).
export default function IcmsScreen() {
  ...
  if (!isWeb) {
    return (
      <LockedView
        ...
        message="Icms está disponível apenas no web."
        testID="icms-web-only"
      />
    );
  }

  const [conn, setConn] = useState<Conn | null>(null);
  const [items, setItems] = useState<Icms[]>([]);
  ...
  const load = useCallback(async (c: Conn, q: string) => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/tabelas/icms?${qs}`);
  ...
  const openEdit = (c: Icms) => { setEditCod(c.codigo); setCodigo(c.codigo); setDescricao(c.descricao); setFormOpen(true); };
  ...
  const save = async () => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/tabelas/icms`, {
      ...
      const j = await r.json();
      if (j?.success) { showToast(j.message || "ICMS gravado."); setFormOpen(false); load(conn, search); }
  ...
  const remove = async (c: Icms) => {
    if (!conn) return;
    try {
      const base = conn.api.replace(/\/+$/, "");
      const r = await fetch(`${base}/api/tabelas/icms/${encodeURIComponent(c.codigo)}/excluir`, {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const canSave = can("ICMS.GRAVAR") || isMaster;
  const canDel = can("ICMS.EXCLUIR") || isMaster;

  return (
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]} testID="icms-screen">
      <View style={styles.header}>
        <Pressable onPress={() => router.back()} hitSlop={12} style={styles.back}>
          <Ionicons name="chevron-back" size={24} color={colors.onBrandPrimary} />
        </Pressable>
        <Image source={require("../assets/images/kontacto-logo.png")} style={{ width: 56, height: 16, marginRight: 8 }} resizeMode="contain" />
        <Text style={styles.headerTitle}>Icms</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={styles.webShell}>
        ...
        <ScrollView contentContainerStyle={[styles.scroll, styles.scrollWeb]}>
          ...
          {!loading && items.length === 0 ? <Text style={styles.empty}>Nenhum ICMS cadastrado.</Text> : null}
          {items.map((c) => (
            <View key={c.codigo} style={styles.row} testID={`icms-${c.codigo}`}>
              <Pressable style={{ flex: 1 }} onPress={() => canSave && openEdit(c)}>
                <Text style={styles.rowTitle}>{c.codigo} · {c.descricao}</Text>
              </Pressable>
              {canDel ? (
                <Pressable onPress={() => remove(c)} hitSlop={8} testID={`icms-del-${c.codigo}`}>
      ...
      {canSave ? (
        <Pressable onPress={openNew} style={styles.fab} testID="icms-novo">
      ...
      <Modal visible={formOpen} transparent animationType="slide" onRequestClose={() => setFormOpen(false)}>
        <Pressable style={styles.modalBg} onPress={() => setFormOpen(false)}>
          <Pressable style={styles.modalCard} onPress={(e) => e.stopPropagation()}>
            <Text style={styles.modalTitle}>{editCod ? `ICMS ${editCod}` : "Novo ICMS"}</Text>

            <Text style={styles.label}>Código *</Text>
            <TextInput
              value={codigo}
              onChangeText={(v) => setCodigo(v.toUpperCase())}
              placeholder="Ex.: 00, 20, 101"
              placeholderTextColor={colors.muted}
            ...
            <TextInput
              ...
              style={styles.input}
              maxLength={35}
              testID="icms-descricao"
            />

            <Pressable onPress={save} disabled={saving} style={[styles.primaryBtn, saving && { opacity: 0.6 }]} testID="icms-salvar">
