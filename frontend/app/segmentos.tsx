...
type Segmento = { codigo: string; descricao: string };

// Cadastro/Tabelas Auxiliares > Segmentos (tabela `segmentos`). Código
// auto-gerado (3 dígitos sequenciais), mesmo padrão de Marcas/Forma de
// Pagamento. Delete guard real: cliente.segmento é gravado por este app.
export default function SegmentosScreen() {
  ...
  if (!isWeb) {
    return (
      <LockedView
        ...
        message="Segmentos está disponível apenas no web."
        testID="segmentos-web-only"
      />
    );
  }

  const [conn, setConn] = useState<Conn | null>(null);
  const [items, setItems] = useState<Segmento[]>([]);
  ...
  const load = useCallback(async (c: Conn, q: string) => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/tabelas/segmentos?${qs}`);
  ...
  const openEdit = (s: Segmento) => { setEditCod(s.codigo); setDescricao(s.descricao); setFormOpen(true); };
  ...
  const save = async () => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/tabelas/segmentos`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ servidor: conn.servidor, banco: conn.banco, ...auditCtx, codigo: editCod, descricao: descricao.trim() }),
      });
      const j = await r.json();
      if (j?.success) { showToast(j.message || "Segmento gravado."); setFormOpen(false); load(conn, search); }
  ...
  const remove = async (s: Segmento) => {
    if (!conn) return;
    try {
      const base = conn.api.replace(/\/+$/, "");
      const r = await fetch(`${base}/api/tabelas/segmentos/${encodeURIComponent(s.codigo)}/excluir`, {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const canSave = can("SEGMENTOS.GRAVAR") || isMaster;
  const canDel = can("SEGMENTOS.EXCLUIR") || isMaster;

  return (
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]} testID="segmentos-screen">
      <View style={styles.header}>
        <Pressable onPress={() => router.back()} hitSlop={12} style={styles.back}>
          <Ionicons name="chevron-back" size={24} color={colors.onBrandPrimary} />
        </Pressable>
        <Image source={require("../assets/images/kontacto-logo.png")} style={{ width: 56, height: 16, marginRight: 8 }} resizeMode="contain" />
        <Text style={styles.headerTitle}>Segmentos</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={styles.webShell}>
        <View style={styles.filterBox}>
          <TextInput
            ...
            style={styles.input}
            testID="segmentos-search"
          />
        </View>

        <ScrollView contentContainerStyle={[styles.scroll, styles.scrollWeb]}>
          {loading ? <ActivityIndicator color={colors.brandPrimary} style={{ marginTop: 24 }} /> : null}
          {!loading && items.length === 0 ? <Text style={styles.empty}>Nenhum segmento cadastrado.</Text> : null}
          {items.map((s) => (
            <View key={s.codigo} style={styles.row} testID={`segmento-${s.codigo}`}>
              <Pressable style={{ flex: 1 }} onPress={() => canSave && openEdit(s)}>
                <Text style={styles.rowTitle}>{s.codigo} · {s.descricao}</Text>
              </Pressable>
              {canDel ? (
                <Pressable onPress={() => remove(s)} hitSlop={8} testID={`segmento-del-${s.codigo}`}>
      ...
      {canSave ? (
        <Pressable onPress={openNew} style={styles.fab} testID="segmentos-novo">
      ...
      <Modal visible={formOpen} transparent animationType="slide" onRequestClose={() => setFormOpen(false)}>
        <Pressable style={styles.modalBg} onPress={() => setFormOpen(false)}>
          <Pressable style={styles.modalCard} onPress={(e) => e.stopPropagation()}>
            <Text style={styles.modalTitle}>{editCod ? `Segmento ${editCod}` : "Novo segmento"}</Text>
            ...
            <TextInput
              ...
              testID="segmento-descricao"
            />
            <Pressable onPress={save} disabled={saving} style={[styles.primaryBtn, saving && { opacity: 0.6 }]} testID="segmento-salvar">
