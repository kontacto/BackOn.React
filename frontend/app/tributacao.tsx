...
type Tributacao = { codigo: string; descricao: string; aplicacao: string; regime_label: string };

// Cadastro/Tabelas Auxiliares > Tributação (tabela `Tributacao` — códigos
// CST/CSOSN de ICMS). Legado: FrmManTri ("Manutenção de Tributações"), campo
// "Código" — código É digitado pelo usuário (upsert-by-codigo, mesmo padrão
// de Situação/Icms/Origem/Status de O.S./Tipo de Pré-Venda/Tipo de Produto/
// Tipo de Serviço), travado depois de criado.
//
// Correção de bug do legado: o Gravar original só salva Descrição — o campo
// "Aplicação" (textarea, editável e mostrado na grid) nunca era persistido.
// Corrigido aqui pra salvar os dois. `Regime_Tributario` (Normal/Simples
// Nacional) não tem controle nesta tela — fica no default (Normal), só
// exibido na listagem, mesmo comportamento do legado.
export default function TributacaoScreen() {
  ...
  if (!isWeb) {
    return (
      <LockedView
        ...
        message="Tributação está disponível apenas no web."
        testID="tributacao-web-only"
      />
    );
  }

  const [conn, setConn] = useState<Conn | null>(null);
  const [items, setItems] = useState<Tributacao[]>([]);
  ...
  const load = useCallback(async (c: Conn, q: string) => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/tabelas/tributacao?${qs}`);
  ...
  const openNew = () => { setEditCod(null); setCodigo(""); setDescricao(""); setAplicacao(""); setFormOpen(true); };
  const openEdit = (t: Tributacao) => {
    setEditCod(t.codigo); setCodigo(t.codigo); setDescricao(t.descricao); setAplicacao(t.aplicacao); setFormOpen(true);
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/tabelas/tributacao`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          servidor: conn.servidor, banco: conn.banco, ...auditCtx,
          codigo: codigo.trim(), descricao: descricao.trim(), aplicacao: aplicacao.trim(),
        }),
      });
      const j = await r.json();
      if (j?.success) { showToast(j.message || "Tributação gravada."); setFormOpen(false); load(conn, search); }
  ...
  const remove = async (t: Tributacao) => {
    if (!conn) return;
    try {
      const base = conn.api.replace(/\/+$/, "");
      const r = await fetch(`${base}/api/tabelas/tributacao/${encodeURIComponent(t.codigo)}/excluir`, {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const canSave = can("TRIBUTACAO.GRAVAR") || isMaster;
  const canDel = can("TRIBUTACAO.EXCLUIR") || isMaster;

  return (
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]} testID="tributacao-screen">
      <View style={styles.header}>
        <Pressable onPress={() => router.back()} hitSlop={12} style={styles.back}>
          <Ionicons name="chevron-back" size={24} color={colors.onBrandPrimary} />
        </Pressable>
        <Image source={require("../assets/images/kontacto-logo.png")} style={{ width: 56, height: 16, marginRight: 8 }} resizeMode="contain" />
        <Text style={styles.headerTitle}>Tributação</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={styles.webShell}>
        <View style={styles.filterBox}>
          <TextInput
            ...
            style={styles.input}
            testID="tributacao-search"
          />
        </View>

        <ScrollView contentContainerStyle={[styles.scroll, styles.scrollWeb]}>
          {loading ? <ActivityIndicator color={colors.brandPrimary} style={{ marginTop: 24 }} /> : null}
          {!loading && items.length === 0 ? <Text style={styles.empty}>Nenhuma tributação cadastrada.</Text> : null}
          {items.map((t) => (
            <View key={t.codigo} style={styles.row} testID={`tributacao-${t.codigo}`}>
              <Pressable style={{ flex: 1 }} onPress={() => canSave && openEdit(t)}>
                <Text style={styles.rowTitle}>{t.codigo} · {t.descricao}</Text>
                <Text style={styles.rowSub}>{t.regime_label}</Text>
              </Pressable>
              {canDel ? (
                <Pressable onPress={() => remove(t)} hitSlop={8} testID={`tributacao-del-${t.codigo}`}>
      ...
      {canSave ? (
        <Pressable onPress={openNew} style={styles.fab} testID="tributacao-novo">
          ...
          <Pressable style={styles.modalCard} onPress={(e) => e.stopPropagation()}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <TextInput
                ...
                style={styles.input}
                maxLength={80}
                testID="tributacao-descricao"
              />
              <Text style={styles.label}>Aplicação</Text>
              <TextInput
                value={aplicacao}
                onChangeText={setAplicacao}
                placeholder="Notas de aplicação deste código"
                placeholderTextColor={colors.muted}
                style={[styles.input, styles.textArea]}
                multiline
                numberOfLines={4}
                testID="tributacao-aplicacao"
              />
              <Pressable onPress={save} disabled={saving} style={[styles.primaryBtn, saving && { opacity: 0.6 }]} testID="tributacao-salvar">
