...
type TipoServico = { codigo: number; descricao: string };

// Cadastro/Tabelas Auxiliares > Tipo de Serviço (tabela `tipo_servico`).
// Legado: FrmManTipoServ ("Manutenção Tipo De Serviços..."), campo "Tipo" —
// código É digitado pelo usuário (upsert-by-codigo, mesmo padrão de
// Situação/Icms/Origem/Status de O.S./Tipo de Pré-Venda/Tipo de Produto),
// travado depois de criado. Sem FK real no banco — `servicos.tipo` é a
// referência de fato (mesmo tipo, valores compatíveis), checada por
// precaução no delete guard.
export default function TipoServicoScreen() {
  ...
  if (!isWeb) {
    return (
      <LockedView
        ...
        message="Tipo de Serviço está disponível apenas no web."
        testID="tipo-servico-web-only"
      />
    );
  }
  ...
  if (!moduleOn("servicos")) {
    return (
      <LockedView
        ...
        testID="tipo-servico-module-off"
      />
    );
  }
  ...
  const load = useCallback(async (c: Conn, q: string) => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/tabelas/tipo-servico?${qs}`);
  ...
  const openEdit = (t: TipoServico) => { setEditCod(t.codigo); setCodigo(String(t.codigo)); setDescricao(t.descricao); setFormOpen(true); };
  ...
  const save = async () => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/tabelas/tipo-servico`, {
  ...
  const remove = async (t: TipoServico) => {
    if (!conn) return;
    try {
      const base = conn.api.replace(/\/+$/, "");
      const r = await fetch(`${base}/api/tabelas/tipo-servico/${t.codigo}/excluir`, {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const canSave = can("TIPO_SERVICO.GRAVAR") || isMaster;
  const canDel = can("TIPO_SERVICO.EXCLUIR") || isMaster;

  return (
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]} testID="tipo-servico-screen">
      <View style={styles.header}>
        <Pressable onPress={() => router.back()} hitSlop={12} style={styles.back}>
          <Ionicons name="chevron-back" size={24} color={colors.onBrandPrimary} />
        </Pressable>
        <Image source={require("../assets/images/kontacto-logo.png")} style={{ width: 56, height: 16, marginRight: 8 }} resizeMode="contain" />
        <Text style={styles.headerTitle}>Tipo de Serviço</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={styles.webShell}>
        ...
        <ScrollView contentContainerStyle={[styles.scroll, styles.scrollWeb]}>
          ...
          {items.map((t) => (
            <View key={t.codigo} style={styles.row} testID={`tipo-servico-${t.codigo}`}>
              <Pressable style={{ flex: 1 }} onPress={() => canSave && openEdit(t)}>
                <Text style={styles.rowTitle}>{t.codigo} · {t.descricao}</Text>
              </Pressable>
              {canDel ? (
                <Pressable onPress={() => remove(t)} hitSlop={8} testID={`tipo-servico-del-${t.codigo}`}>
      ...
      {canSave ? (
        <Pressable onPress={openNew} style={styles.fab} testID="tipo-servico-novo">
        ...
        <Pressable style={styles.modalBg} onPress={() => setFormOpen(false)}>
          <Pressable style={styles.modalCard} onPress={(e) => e.stopPropagation()}>
            ...
            <TextInput
              ...
              testID="tipo-servico-descricao"
            />
            <Pressable onPress={save} disabled={saving} style={[styles.primaryBtn, saving && { opacity: 0.6 }]} testID="tipo-servico-salvar">
