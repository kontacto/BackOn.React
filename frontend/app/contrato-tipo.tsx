...
// Transações > Contratos > Tipo de Contrato (tabela `tipo_contrato`,
// migração de FrmManTPC.frm) — mesmo padrão de tipo-cliente.tsx.
export default function ContratoTipoScreen() {
  ...
  if (!isWeb) {
    return (
      <LockedView
        ...
        message="Tipo de Contrato está disponível apenas no web."
        testID="contrato-tipo-web-only"
      />
    );
  }
  ...
  if (!moduleOn("contratos")) {
    return (
      <LockedView
        ...
        testID="contrato-tipo-module-off"
      />
    );
  }
  ...
  const load = useCallback(async (c: Conn, q: string) => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/contratos/tipo-contrato?${qs}`);
  ...
  const save = async () => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/contratos/tipo-contrato`, {
  ...
  const remove = async (t: Tipo) => {
    ...
    try {
      const base = conn.api.replace(/\/+$/, "");
      const r = await fetch(`${base}/api/contratos/tipo-contrato/${t.codigo}/excluir`, {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const canSave = can("TIPO_CONTRATO.GRAVAR") || isMaster;
  const canDel = can("TIPO_CONTRATO.EXCLUIR") || isMaster;

  return (
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]} testID="contrato-tipo-screen">
      <View style={styles.header}>
        <Pressable onPress={() => router.back()} hitSlop={12} style={styles.back}>
          <Ionicons name="chevron-back" size={24} color={colors.onBrandPrimary} />
        </Pressable>
        <Text style={styles.headerTitle}>Tipo de Contrato</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={styles.webShell}>
        ...
        <ScrollView contentContainerStyle={[styles.scroll, styles.scrollWeb]}>
          ...
          {items.map((t) => (
            <View key={t.codigo} style={styles.row} testID={`contrato-tipo-${t.codigo}`}>
              <Pressable style={{ flex: 1 }} onPress={() => canSave && openEdit(t)}>
                <Text style={styles.rowTitle}>{t.codigo} · {t.descricao}</Text>
              </Pressable>
              {canDel ? (
                <Pressable onPress={() => remove(t)} hitSlop={8} testID={`contrato-tipo-del-${t.codigo}`}>
      ...
      {canSave ? (
        <Pressable onPress={openNew} style={styles.fab} testID="contrato-tipo-novo">
        ...
        <Pressable style={styles.modalBg} onPress={() => setFormOpen(false)}>
          <Pressable style={styles.modalCard} onPress={(e) => e.stopPropagation()}>
            ...
            <TextInput
              ...
              placeholder="Ex.: Manutenção 10hs"
              placeholderTextColor={colors.muted}
              style={styles.input}
              maxLength={30}
              autoCapitalize="characters"
              testID="contrato-tipo-descricao"
            />
            <Pressable onPress={save} disabled={saving} style={[styles.primaryBtn, saving && { opacity: 0.6 }]} testID="contrato-tipo-salvar">
