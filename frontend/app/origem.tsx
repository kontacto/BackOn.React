...
type Origem = { codigo: string; descricao: string };

// Cadastro/Tabelas Auxiliares > Origem (tabela `origem`) — "Origem da
// Mercadoria" padrão NFe (dígito 0-8). Código é o próprio dígito fiscal
// padronizado, digitado pelo usuário — upsert-by-codigo, travado depois de criado.
export default function OrigemScreen() {
  ...
  if (!isWeb) {
    return (
      <LockedView
        ...
        message="Origem está disponível apenas no web."
        testID="origem-web-only"
      />
    );
  }

  const [conn, setConn] = useState<Conn | null>(null);
  const [items, setItems] = useState<Origem[]>([]);
  ...
  const openEdit = (o: Origem) => { setEditCod(o.codigo); setCodigo(o.codigo); setDescricao(o.descricao); setFormOpen(true); };
  ...
  const save = async () => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/tabelas/origem`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ servidor: conn.servidor, banco: conn.banco, ...auditCtx, codigo: codigo.trim(), descricao: descricao.trim() }),
      });
      const j = await r.json();
      if (j?.success) { showToast(j.message || "Origem gravada."); setFormOpen(false); load(conn, search); }
  ...
  const remove = async (o: Origem) => {
    if (!conn) return;
    try {
      const base = conn.api.replace(/\/+$/, "");
      const r = await fetch(`${base}/api/tabelas/origem/${encodeURIComponent(o.codigo)}/excluir`, {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const canSave = can("ORIGEM.GRAVAR") || isMaster;
  const canDel = can("ORIGEM.EXCLUIR") || isMaster;

  return (
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]} testID="origem-screen">
      <View style={styles.header}>
        <Pressable onPress={() => router.back()} hitSlop={12} style={styles.back}>
          <Ionicons name="chevron-back" size={24} color={colors.onBrandPrimary} />
        </Pressable>
        <Image source={require("../assets/images/kontacto-logo.png")} style={{ width: 56, height: 16, marginRight: 8 }} resizeMode="contain" />
        <Text style={styles.headerTitle}>Origem</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={styles.webShell}>
        <View style={styles.filterBox}>
          <TextInput
            ...
            style={styles.input}
            testID="origem-search"
          />
        </View>

        <ScrollView contentContainerStyle={[styles.scroll, styles.scrollWeb]}>
          {loading ? <ActivityIndicator color={colors.brandPrimary} style={{ marginTop: 24 }} /> : null}
          {!loading && items.length === 0 ? <Text style={styles.empty}>Nenhuma origem cadastrada.</Text> : null}
          {items.map((o) => (
            <View key={o.codigo} style={styles.row} testID={`origem-${o.codigo}`}>
              <Pressable style={{ flex: 1 }} onPress={() => canSave && openEdit(o)}>
                <Text style={styles.rowTitle}>{o.codigo} · {o.descricao}</Text>
              </Pressable>
              {canDel ? (
                <Pressable onPress={() => remove(o)} hitSlop={8} testID={`origem-del-${o.codigo}`}>
      ...
      {canSave ? (
        <Pressable onPress={openNew} style={styles.fab} testID="origem-novo">
      ...
      <Modal visible={formOpen} transparent animationType="slide" onRequestClose={() => setFormOpen(false)}>
        <Pressable style={styles.modalBg} onPress={() => setFormOpen(false)}>
          <Pressable style={styles.modalCard} onPress={(e) => e.stopPropagation()}>
            <Text style={styles.modalTitle}>{editCod ? `Origem ${editCod}` : "Nova origem"}</Text>
            ...
            <TextInput
              ...
              placeholder="Ex.: Mercadorias Nacionais"
              placeholderTextColor={colors.muted}
              style={styles.input}
              testID="origem-descricao"
            />

            <Pressable onPress={save} disabled={saving} style={[styles.primaryBtn, saving && { opacity: 0.6 }]} testID="origem-salvar">
