...
type Area = { codigo: number; descricao: string };

// ============================================================
// Tela — Manutenção de Áreas (web-only), tabela `area` (estoque:
// Loja/Depósito etc., referenciada por pecas.area). Não confundir com
// `area_atuacao` (classificação de Pedido/O.S.), que tem seu próprio lookup.
// ============================================================
export default function AreaScreen() {
  ...
  if (!isWeb) {
    return (
      <LockedView
        ...
        message="Áreas está disponível apenas no web."
        testID="area-web-only"
      />
    );
  }
  ...
  const load = useCallback(async (c: Conn) => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/tabelas/area?servidor=${encodeURIComponent(c.servidor)}&banco=${encodeURIComponent(c.banco)}`);
  ...
  const openEdit = (a: Area) => { setEditCod(a.codigo); setDescricao(a.descricao); setFormOpen(true); };
  ...
  const remove = async (a: Area) => {
    ...
    try {
      const base = conn.api.replace(/\/+$/, "");
      const r = await fetch(`${base}/api/tabelas/area/${a.codigo}/excluir`, {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const canSave = can("AREA.GRAVAR") || isMaster;
  const canDel = can("AREA.EXCLUIR") || isMaster;

  return (
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]} testID="area-screen">
      ...
      <ScrollView contentContainerStyle={[styles.scroll, isWeb && styles.scrollWeb]}>
        <View style={isWeb ? styles.webShell : undefined}>
          ...
          {!loading && items.length === 0 ? <Text style={styles.empty}>Nenhuma área cadastrada.</Text> : null}
          {items.map((a) => (
            <View key={a.codigo} style={styles.row} testID={`area-${a.codigo}`}>
              <Pressable style={{ flex: 1 }} onPress={() => canSave && openEdit(a)}>
                <Text style={styles.rowTitle}>{a.codigo} · {a.descricao}</Text>
              </Pressable>
              {canDel ? (
                <Pressable onPress={() => remove(a)} hitSlop={8} testID={`area-del-${a.codigo}`}>
      ...
      {canSave ? (
        <Pressable onPress={openNew} style={styles.fab} testID="area-novo">
      ...
      <Modal visible={formOpen} transparent animationType="slide" onRequestClose={() => setFormOpen(false)}>
        <Pressable style={styles.modalBg} onPress={() => setFormOpen(false)}>
          <Pressable style={styles.modalCard} onPress={(e) => e.stopPropagation()}>
            <Text style={styles.modalTitle}>{editCod != null ? `Área ${editCod}` : "Nova área"}</Text>
            ...
            <TextInput
              ...
              style={styles.input}
              maxLength={30}
              testID="area-descricao"
            />
            <Pressable onPress={save} disabled={saving} style={[styles.primaryBtn, saving && { opacity: 0.6 }]} testID="area-salvar">
