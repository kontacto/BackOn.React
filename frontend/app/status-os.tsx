...
type StatusOs = { codigo: number; descricao: string };

// Cadastro/Tabelas Auxiliares > Status de O.S. (tabela `status_os`). Legado:
// FrmManStaOs, campo "Tipo" — código É digitado pelo usuário (upsert-by-
// codigo, mesmo padrão de Situação/Icms/Origem), travado depois de criado.
// Delete guard real: `os.status_os` é gravado de verdade por este app
// (campo próprio, distinto de os.situacao).
export default function StatusOsScreen() {
  ...
  if (!isWeb) {
    return (
      <LockedView
        ...
        message="Status de O.S. está disponível apenas no web."
        testID="status-os-web-only"
      />
    );
  }

  const [conn, setConn] = useState<Conn | null>(null);
  const [items, setItems] = useState<StatusOs[]>([]);
  ...
  const load = useCallback(async (c: Conn, q: string) => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/tabelas/status-os?${qs}`);
  ...
  const openEdit = (s: StatusOs) => { setEditCod(s.codigo); setCodigo(String(s.codigo)); setDescricao(s.descricao); setFormOpen(true); };
  ...
  const save = async () => {
    ...
    try {
      ...
      const r = await fetch(`${base}/api/tabelas/status-os`, {
      ...
      const j = await r.json();
      if (j?.success) { showToast(j.message || "Status gravado."); setFormOpen(false); load(conn, search); }
  ...
  const remove = async (s: StatusOs) => {
    if (!conn) return;
    try {
      const base = conn.api.replace(/\/+$/, "");
      const r = await fetch(`${base}/api/tabelas/status-os/${s.codigo}/excluir`, {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const canSave = can("STATUS_OS.GRAVAR") || isMaster;
  const canDel = can("STATUS_OS.EXCLUIR") || isMaster;

  return (
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]} testID="status-os-screen">
      <View style={styles.header}>
        <Pressable onPress={() => router.back()} hitSlop={12} style={styles.back}>
          <Ionicons name="chevron-back" size={24} color={colors.onBrandPrimary} />
        </Pressable>
        <Image source={require("../assets/images/kontacto-logo.png")} style={{ width: 56, height: 16, marginRight: 8 }} resizeMode="contain" />
        <Text style={styles.headerTitle}>Status de O.S.</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={styles.webShell}>
        <View style={styles.filterBox}>
          <TextInput
            ...
            style={styles.input}
            testID="status-os-search"
          />
        </View>

        <ScrollView contentContainerStyle={[styles.scroll, styles.scrollWeb]}>
          {loading ? <ActivityIndicator color={colors.brandPrimary} style={{ marginTop: 24 }} /> : null}
          {!loading && items.length === 0 ? <Text style={styles.empty}>Nenhum status cadastrado.</Text> : null}
          {items.map((s) => (
            <View key={s.codigo} style={styles.row} testID={`status-os-${s.codigo}`}>
              <Pressable style={{ flex: 1 }} onPress={() => canSave && openEdit(s)}>
                <Text style={styles.rowTitle}>{s.codigo} · {s.descricao}</Text>
              </Pressable>
              {canDel ? (
                <Pressable onPress={() => remove(s)} hitSlop={8} testID={`status-os-del-${s.codigo}`}>
      ...
      {canSave ? (
        <Pressable onPress={openNew} style={styles.fab} testID="status-os-novo">
      ...
      <Modal visible={formOpen} transparent animationType="slide" onRequestClose={() => setFormOpen(false)}>
        <Pressable style={styles.modalBg} onPress={() => setFormOpen(false)}>
          <Pressable style={styles.modalCard} onPress={(e) => e.stopPropagation()}>
            <Text style={styles.modalTitle}>{editCod != null ? `Status ${editCod}` : "Novo status"}</Text>
            ...
            <TextInput
              ...
              testID="status-os-descricao"
            />
            <Pressable onPress={save} disabled={saving} style={[styles.primaryBtn, saving && { opacity: 0.6 }]} testID="status-os-salvar">
