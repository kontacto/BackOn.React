"""Testes UNITÁRIOS de Tanque/Nota Fiscal (Posto de Combustível)."""
import services.tanque_nf_service as svc
...
class TestValidacoesSemBanco:
    def test_nota_obrigatoria(self):
        r = svc._save_sync("srv", "bd", nota=None, tanque=1, qtd=1000)
        assert r["success"] is False and "nota fiscal" in r["message"].lower()

    def test_tanque_obrigatorio(self):
        r = svc._save_sync("srv", "bd", nota=1, tanque=None, qtd=1000)
        assert r["success"] is False and "tanque" in r["message"].lower()

    def test_qtd_obrigatoria(self):
        r = svc._save_sync("srv", "bd", nota=1, tanque=1, qtd=0)
...
class TestModuloDesativado:
    def test_save_bloqueado(self, monkeypatch):
        ...
        r = svc._save_sync("srv", "bd", nota=1, tanque=1, qtd=1000)
...
class TestSaveComMock:
    def test_nota_nao_encontrada(self, monkeypatch):
        ...
        conn = _patch(monkeypatch, cur)
        r = svc._save_sync("srv", "bd", nota=99, tanque=1, qtd=1000)
        assert r["success"] is False and "nota fiscal" in r["message"].lower()
        assert conn.committed is False
    ...
    def test_tanque_nao_encontrado(self, monkeypatch):
        ...
        conn = _patch(monkeypatch, cur)
        r = svc._save_sync("srv", "bd", nota=1, tanque=99, qtd=1000)
        assert r["success"] is False and "tanque" in r["message"].lower()
        assert conn.committed is False
    ...
    def test_insere_novo(self, monkeypatch):
        ...
        assert any("INSERT INTO Tanque_NF" in q for q, _ in cur.queries)

    def test_atualiza_existente(self, monkeypatch):
        cur = FakeCursor(one=[MODULO_ON, {"ok": 1}, {"ok": 1}, {"ok": 1}])
        conn = _patch(monkeypatch, cur)
        r = svc._save_sync("srv", "bd", nota=1, tanque=1, qtd=1500)
        assert r["success"] is True
        assert conn.committed is True
        assert any("UPDATE Tanque_NF" in q for q, _ in cur.queries)
...
class TestDeleteComMock:
    def test_exclui(self, monkeypatch):
        ...
        conn = _patch(monkeypatch, cur)
        r = svc._delete_sync("srv", "bd", nota=1, tanque=1)
        assert r["success"] is True
        assert conn.committed is True
